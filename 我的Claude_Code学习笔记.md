...
**核心发现**:
- 7层架构设计：从用户界面到基础设施的完整流水线
- 智能并发控制：最大10个工具同时执行 (`gW5 = 10`)
- 流式处理机制：实时响应用户，边做边展示
- 智能消息压缩：`wU2`函数自动优化对话历史

**执行流程图**:
```
用户输入 → 输入捕获(jO2) → Agent主循环(nO) → LLM API(wu) → 工具执行(MH1) → 结果返回
```

**技术映射表**:
| 混淆名称 | 功能 | 位置 |
|---------|------|------|
| `nO` | Agent主循环 | cli.beautify.mjs:284675 |
| `wu` | 会话流生成器 | cli.beautify.mjs:282537 |
| `wU2` | 消息压缩器 | cli.beautify.mjs:284329 |
| `MH1` | 工具执行引擎 | cli.beautify.mjs:284824 |
